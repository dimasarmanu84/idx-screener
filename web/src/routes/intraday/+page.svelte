...
<div class="py-3 md:py-8">
	...
	<div class="mb-2 md:mb-4">
		<div class="flex items-center gap-2">
			...
			<ModeGuide mode="intraday" />
		</div>
		<p class="hidden text-sm text-gray-500 md:mt-1 md:block dark:text-terminal-muted">
			Beli Pagi Jual Sore — Day trading dengan filter likuiditas ketat
		</p>
	</div>
	...
	<div class="mb-2 flex flex-wrap items-center gap-2 md:mb-5 md:gap-3">
		{#if manualMode}
			<div class="flex flex-1 items-center gap-2 sm:flex-none">
				<input
					...
					class="h-7 w-full rounded-md border border-gray-300 bg-white px-2 font-mono text-[11px] uppercase placeholder:normal-case placeholder:font-sans placeholder:text-gray-400 focus:border-green-500 focus:ring-1 focus:ring-green-500 md:h-10 md:rounded-lg md:px-3 md:text-sm dark:border-terminal-border dark:bg-terminal-bg dark:text-terminal-text dark:placeholder:text-terminal-dim sm:w-72"
				/>
			</div>
		{:else}
		...
		{:else}
			<button
				onclick={startScan}
				class="flex h-7 items-center gap-1 rounded-md bg-green-600 px-2.5 text-[11px] font-semibold text-white shadow-sm shadow-green-500/25 transition-all hover:bg-green-700 active:scale-[0.98] md:h-10 md:gap-2 md:rounded-lg md:px-5 md:text-sm"
			>
				<svg class="h-3 w-3 md:h-4 md:w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
